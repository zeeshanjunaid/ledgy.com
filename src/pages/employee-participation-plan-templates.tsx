...
import { TemplateFAQs, Testimonials, Instructions, CallToAction, DigitalSignatures, Video } from "../components/templates";
...
const EquityPlans = (props: Props) => {
  ...
  const buttonOne = <Button href={`${appUrl}/templates`} onClick={() => track('click.templates')} className="mr-2 mb-2 btn-red">
      <Trans>Get started</Trans>
    </Button>;

  const productHuntLaunchButton = <ProductHuntButton productHuntLink="https://www.producthunt.com/posts/employee-participation-plan-templates?utm_source=badge-featured&utm_medium=badge&utm_souce=badge-startup-fundraising-calculator" trackSignupKey="clickProductHuntESOP" postId="175968" altText="Free Employee Participation Plan Templates - Using Ledgy! | Product Hunt Embed" />;
  const isGerman = lang === 'de';
  return <>
      <Title title={title} description={description} thumbnailUrl={`${siteUrl}/banners/employee-participation-plans.png`} />
      <MainHeaderLayout title={title} subtitle={description} buttonOne={buttonOne} customButton={productHuntLaunchButton} image={<Img {...isGerman ? data.templatesDe : data.templates} alt="Templates" />} />
      <ContentBody>
        <>
          <Instructions />
          <CallToAction />
