...
const FooterCol = ({
  order,
  children
}: {order: number;children: Node;}) => <div className={`col-6 col-md-3 order-md-${order}`}>{children}</div>;
...
const LanguageLink = ({
  language,
  to
}: {language: string;to: string;}) => <Link className="d-flex justify-content-center text-primary" to={to} href>
    {language}
  </Link>;
          ...
          <div className="row m-0 h-100 justify-content-md-center position-relative z-index-base">
            <FooterCol order={2}>
              <FooterColBody title={<Trans>Company</Trans>}>
                {companyLinks.map(([label, link]) => <Link className="nav-link" href to={formatUrl(prefix, link)} key={link}>
                    {label}
                  </Link>)}
              </FooterColBody>
              <FooterColBody title={<Trans>Legal</Trans>} className="mt-5">
                {legalLinks.map(([label, link]) => <Link className="nav-link" href to={formatUrl(prefix, link)} key={link}>
                    {label}
                  </Link>)}
              </FooterColBody>
            </FooterCol>
            <FooterCol order={3}>
              <FooterColBody title={<Trans>Resources</Trans>}>
                {resourceLinks.map(([label, link]) => isExternalUrl(link) ? <a className="nav-link" href={link} key={link} {...targetBlank}>
                      {label}
                    </a> : <Link className="nav-link" href to={formatUrl(prefix, link)} key={link}>
                      {label}
                    </Link>)}
              </FooterColBody>
            </FooterCol>
              ...
              <div className="footer--logo-section d-flex flex-column justify-content-between mt-5 mt-md-0">
                <div className="d-flex flex-column align-items-center p-0 px-md-4">
                  ...
                  <div className="py-lg-4">
                    ...
                    <Dropdown toggleText={<Trans>Language</Trans>} toggleIcon={faGlobeEurope} toggleClass="w-100" toggleProps={{ outline: true }} items={[<LanguageLink language="English" to={deprefix(location.pathname)} />, <LanguageLink language="Deutsch" to={`/de${deprefix(location.pathname)}`} />, <LanguageLink language="Français" to={`/fr${deprefix(location.pathname)}`} />]} />
                  </div>
                </div>
                <div className="d-flex justify-content-center flex-wrap mt-5">
                  {socialLinks.map(([href, icon, title]) => <a href={href} key={title} {...targetBlank} className="social-icon text-white mx-2">
            ...
            <FooterCol order={4}>
              <FooterColBody title={<Trans>Product</Trans>} className="mt-5 mt-md-0">
                {productLinks.map(([label, link]) => <Link className="nav-link" href to={formatUrl(prefix, link)} key={link}>
                    {label}
                  </Link>)}
              </FooterColBody>
            </FooterCol>
