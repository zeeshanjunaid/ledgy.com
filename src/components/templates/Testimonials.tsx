...
export const Testimonials = ({
  ...
  const testimonials = TESTIMONIALS.map(({
    ...
    img,
    rounded
  }) => <Testimonial col={rounded ? 3 : 5} key={name} name={name} url={url} img={img} description={description} rounded={rounded} />);

  return <div className="py-6">
      <div className="row text-center justify-content-center mb-3">{testimonials}</div>
      <div className="row text-center justify-content-center">
        <Testimonial col={10} name={t`Dominic Jacquesson, VP Talent @ Index Ventures, a VC who founded the Not Optional initiative`} img={data.notoptional} url="https://notoptional.eu/en/" minHeight={0} description={<Trans>
