...
const MobileNavbarGroup = ({
  ...
  toggleOverlay: (arg0: boolean, arg1: (arg0: boolean) => void) => void;
    ...
    <ul className="flex-column ml-2 ml-sm-4 mb-2 mb-sm-4">
      ...
      const externalLink = <a href={to} key={to} onClick={() => toggleOverlay(isOpen, setOpen)} {...targetBlank}>
            {itemContent}
          </a>;
      const internalLink = <Link href to={formatUrl(prefix, to)} onClick={() => toggleOverlay(isOpen, setOpen)}>
            {itemContent}
          </Link>;
...
const titlesAndLinks = [[featuresTitle, features], [resourcesTitle, resources], [pricingTitle, pricing], [dataProtectionTitle, dataProtection]];
    ...
    <div className="mobile-inner">
      <div className="d-flex flex-column align-items-center justify-content-center p-2 p-sm-4 mt-3 mt-sm-4">
        <div className="text-left ml-n2 ml-sm-n4">
          {titlesAndLinks.map(([title, links], i) => <MobileNavbarGroup title={title} links={links} prefix={prefix} isOpen={isOpen} setOpen={setOpen} toggleOverlay={toggleOverlay} key={i} // eslint-disable-line react/no-array-index-key
        />)}
        </div>
