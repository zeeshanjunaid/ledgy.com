...
Let’s say we want to create an option pool of **10%** (in the fully diluted cap table after the capital increase), i.e. the $\textbf{poolShares}$ have to be equal to 10% of the total number of shares after the investment. In this case, the following equation is true:
$$
\text{(Sheryl + Elon+ Peter)’ shares} = \textbf{90\%} * (\text{(Sheryl + Elon+ Peter)’ shares} + \textbf{poolShares})
$$

Simply reshuffling and isolating the terms, we have:

$$
\textbf{poolShares} = \text{(Sheryl + Elon+ \textbf{Peter})’ shares} * \left( \frac{1}{90\%} - 1 \right)
$$
