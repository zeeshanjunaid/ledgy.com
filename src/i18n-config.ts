...
const deprefix = pathname => pathname[0] === '/' && pathname[3] === '/' ? pathname.substr(3) : pathname;
...
const getLocale = () => compose(compact, flatten, values, pick(browserLanguagePropertyKeys))(window.navigator).map(s => s.substr(0, 2)).find(s => languages.includes(s)) || defaultLanguage;
